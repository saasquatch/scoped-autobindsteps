...
export const autoBindSteps = (
  ...
  const registerStep = (
    ...
    scopedSteps.push({ stepMatcher, stepFunction });
  };
      ...
      scenarios.forEach((scenario) => {
        test(scenario.title, (options) => {
          scenario.steps.forEach((step, stepIndex) => {
            const matches = scopedSteps.filter((scopedStep) =>
              matchSteps(step.stepText, scopedStep.stepMatcher)
            );
